...
class FreeBetPage(BaseFrame):
    ...
    def select_person(self, person_name):
        ...
        self.free_bet_calculator()

    def free_bet_calculator(self):
        df,data = df_data()
        ...
        ttk.Label(self, text="Game Name").grid(row=1, column=0, pady=5, padx=10,sticky=tk.E)
        self.game_name_entry = ttk.Entry(self, width=30)
        self.game_name_entry.grid(row=1, column=1, pady=5)


        # Label for 'Bolag'
        ttk.Label(self, text="Bolag").grid(row=2, column=0, pady=5,padx=10, sticky=tk.E)

        # Create a Combobox for 'Bolag'

        # Filtering 'Bolag' with 'free bet'
        free_bet_df = df[df['Bonusinformation'].str.contains('free bet', case=False, na=False)]
        free_bet_bolag = free_bet_df['Bolag'].tolist()

        # Assuming 'self' is an instance of a Tkinter widget or window
        self.bolag_combobox = ttk.Combobox(self, values=free_bet_bolag, width=30)
        self.bolag_combobox.grid(row=2, column=1, pady=5)


    
        ttk.Label(self, text="Total Amount").grid(row=3, column=0, pady=5,padx=10, sticky=tk.E)
        self.amount_entry = ttk.Entry(self, width=30)
        self.amount_entry.grid(row=3, column=1, pady=5)


        ttk.Label(self, text="Back Odds").grid(row=4, column=0, pady=5,padx=10, sticky=tk.E)
        self.back_odds_entry = ttk.Entry(self, width=30)
        self.back_odds_entry.grid(row=4, column=1, pady=5)

        ttk.Label(self, text="Lay Odds").grid(row=5, column=0, pady=5,padx=10, sticky=tk.E)
    ...
    def save_bet(self):
        ...
        outcomes = self.free_bet_outcomes(back_odds, lay_odds, lay_commission, amount)
    ...
    def calculate_bet(self):
        ...
        self.loss_if_back_wins_label.config(text="Loss from fees: {:.2f}".format(-1*outcomes['loss_if_back_wins']))
        self.loss_if_lay_wins_label.config(text="Win after fees: {:.2f}".format(-1*outcomes['loss_if_lay_wins']))




    def free_bet_outcomes(self, back_odds, lay_odds, lay_commission, stake):
        lay_stake = ((back_odds - 1) / (lay_odds - lay_commission)) * stake
        liability_at_exchange = lay_stake * (lay_odds - 1)
        loss_if_back_wins = -1 * ((stake * back_odds - stake) - liability_at_exchange)+stake
        win_after_fees = -1 * ((stake * back_odds - stake) - liability_at_exchange)-stake
        commission_on_lay_win = (lay_stake * (1 - 1 / lay_odds)) * lay_commission
        loss_if_lay_wins = -1*lay_stake * (1 - lay_commission)+stake
